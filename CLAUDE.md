...
**Tool counts by configuration (REGISTERED counts, live-verified via `WP_Abilities_Registry` on Elementor 4.1.4 + Pro 4.1.2; Pro+atomic re-verified 2026-07-05 = 138 emcp + 3 core = 141 after the v3.1.0 Gutenberg block tools; each total = `emcp-tools/*` + the 3 surfaced `core/*` abilities):**
- Free Elementor only: **117** (114 emcp + 3 core) — **93** active by default
- Free Elementor + Elementor 4.0+ atomic: **131** (128 + 3) — **107** active by default
- With Elementor Pro: **127** (124 + 3) — **88** active by default
- With Elementor Pro + Elementor 4.0+: **141** (138 + 3) — **102** active by default
- With Pro + WooCommerce + Elementor 4.0+: **141** — WooCommerce widgets are reached through `add-pro-widget` (catalog tier `woo`), so they add **no** new tools.
...
> **These are REGISTERED counts** (without ACF; when Advanced Custom Fields free/Pro is active, **+7 ACF tools** register on top — 4 reads on by default, 3 writes off, v3.3.0). **42 tools ship disabled-by-default** — SEO & Accessibility (**7**, Pro), Widget Builder (**8**, Pro), PHP Snippets / Sandbox (**6**, free), the **9** Plugins & Themes write tools, **1** `delete-media`, the **2** Users write tools, the **3** Filesystem write tools, the **3** Database write tools, and the **3** ACF write tools (env-gated). So the typical **active** surface is up to 42 smaller until a user enables them on the Tools tab (e.g. Pro + Elementor 4.0+ = **141** registered, **102** active by default). Verified reconciliation: 141 registered − 12 currently-off writes (SEO/WB/PHP-Snippets) − 3 filesystem writes − 3 database writes = 123 in the live `tools/list` when SEO/Widget-Builder/PHP-Snippets are manually enabled.
>
...
### ACF (Advanced Custom Fields) — 7 tools (v3.3.0)

Read/write ACF field values on posts and options pages, discover field groups, and author field groups over MCP — built on the public ACF API (`acf_get_field_groups`, `acf_get_fields`, `get_field`, `update_field`, `acf_import_field_group`, `acf_update_field_group`/`acf_update_field`). The group **only registers when ACF (free or Pro) is active** (`EMCP_Tools_ACF_Abilities::acf_active()` gate in the registrar; every slug excluded from the drift guard via `EMCP_Tools_Admin::acf_tool_slugs()`); Pro-only features (options pages; repeater/flexible_content/gallery/clone field types) degrade gracefully on free ACF (`is_pro()` via `acf_get_setting('pro')`). Writes always resolve field **names → keys** before `update_field()` (by-name writes silently fail on targets with no stored value); flexible-content rows are validated against the field's layouts (`acf_fc_layout`). Deliberately conservative: **no delete tool**, fields can never be removed, a field's `name`/`key`/`type` can never change, and code-registered (acf-json/PHP, `local` flag) groups are refused by `update-acf-field-group`. Reads enabled by default; the 3 writes ship disabled-by-default (defaults marker v12). Admin category `wp_acf` (WordPress tab). Implemented in `includes/abilities/class-acf-abilities.php`; standalone public tests in `tests/` (`vendor/bin/phpunit -c tests/phpunit.xml` — runs without the private `pro/tests` submodule).
