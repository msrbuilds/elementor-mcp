...
2. **`uae-read` / `uae-write`** — **Ultimate Addons for Elementor (UAE)**, *formerly Header Footer Elementor*. Brainstorm Force renamed it; the wordpress.org **slug is still `header-footer-elementor`** and every internal identifier still says HFE (`Header_Footer_Elementor`, `HFE_VER`, the `elementor-hf` CPT, `ehf_*` meta, the `hfe-` widget prefix), so **detection keys off those while user-facing naming follows the product**. UAE is **both natures at once**: 7 free widgets (`hfe-*`) + 50+ in UAE Pro (`uael-*`), AND header/footer/block templates with display conditions. So `EMCP_Tools_UAE_Integration` **extends** `EMCP_Tools_Addon_Pack_Integration` and its read dispatcher carries `list-widgets`/`get-widget-schema` alongside `list-templates`/`get-template`; the write dispatcher does `create-template`/`update-template`/`set-display-conditions`/`delete-template` (confirm-gated). Template CONTENT is built with the normal Elementor tools against the returned `template_id` (same split as EMCP Themer). `get_ability_names()` on the pack base was un-`final`ed so a pack that is also a data plugin can add a write dispatcher.
