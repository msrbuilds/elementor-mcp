...
- **Compact tool mode (dispatcher) — v3.2.0, opt-in.** Option `emcp_tools_dispatcher_mode` (Connection tab, default OFF). When ON, `register_mcp_server()` surfaces only **3 meta-tools** — `emcp-tools/list-tools` (compact catalog: name/description/category/destructive, `search`/`category` filters), `emcp-tools/get-tool-schema` (batch input schemas by name), `emcp-tools/call-tool` (run a tool by name+arguments) — instead of the ~141 individual tools, to escape client tool-count caps. `call-tool` **delegates to each target ability's own `check_permissions()`** (no privilege escalation) and refuses disabled/unknown tools; the individual abilities stay registered (reachable via `call-tool`). `EMCP_Tools_Dispatcher_Abilities` (`includes/abilities/class-dispatcher-abilities.php`) with `active_names()`/`resolve_ability()` seams; registered by the ability registrar but kept out of `ability_names`. Obsoletes Low-tools mode. The discovery context (server description + `list-tools`' `context`) also carries an environment + active-plugin inventory via `EMCP_Tools_Site_Context::environment_summary()`.
