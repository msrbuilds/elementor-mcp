...
- **A focused MCP toolset** covering the full Elementor page-building workflow — and, as of v3.0.0, growing beyond Elementor into general WordPress management across multiple domains: content, settings, plugins & themes, media library, users, performance, filesystem, and database. As of v3.0.0 the 62 per-widget tools were folded into a catalog-backed model, so the active surface is much smaller — every widget is still reachable via discover → inspect → act. Counts scale with your environment (registered counts, live-verified on Elementor 4.1.4):
  ...
  - **+7 ACF tools** when Advanced Custom Fields (free or PRO) is active — they only register alongside ACF (v3.3.0)
  - **42 of these ship disabled-by-default** (SEO & Accessibility, Widget Builder, PHP Snippets, the 9 Plugins & Themes write tools, `delete-media`, the 2 Users write tools, the 3 Filesystem write tools, the 3 Database write tools, and the 3 ACF write tools), so the typical active surface is smaller until you opt in on the Tools tab
...
- **ACF / ACF PRO (beyond Elementor)** — Read and write Advanced Custom Fields values on posts and options pages, discover field groups (the recursive field tree an agent inspects before writing), and author field groups programmatically. Full Pro field support: repeaters, flexible content (layout-validated), galleries, groups, and clones round-trip as nested JSON. Only registers when ACF is active; writes resolve field names to keys, ship disabled-by-default, and there is deliberately no delete and no field rename (renames orphan stored values). (v3.3.0)
...
### ACF (Advanced Custom Fields) — beyond Elementor (7 tools, v3.3.0)

Read and write ACF field values, discover field groups, and author field groups over MCP — built on the public ACF API (`acf_get_field_groups`, `get_field`, `update_field`, `acf_import_field_group`). The whole group **only registers when ACF (free or PRO) is active**; Pro-only capabilities (options pages; repeater / flexible content / gallery / clone fields) degrade gracefully on free ACF. Writes always resolve field names to field keys first (writing by name silently fails on posts with no stored value), and complex Pro fields round-trip as nested JSON arrays. Deliberately conservative authoring: no delete tool, fields can never be removed, and a field's `name`/`key`/`type` can never change (renames orphan stored values). Code-registered groups (acf-json / PHP) are listed with a `local` flag and refused by the group editor. The 4 read tools are **enabled by default**; `update-acf-fields`, `create-acf-field-group`, and `update-acf-field-group` ship **disabled-by-default** (admin opts in on the Tools tab).
