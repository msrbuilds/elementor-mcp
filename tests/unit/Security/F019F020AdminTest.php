 ...
 *   EMCP_TOOLS_DIR . 'bin' . DIRECTORY_SEPARATOR . 'mcp-proxy.mjs'
 ...
 *
 *   F-020 BEFORE fix → full EMCP_TOOLS_DIR path present in localize_script data.
...
class F019F020AdminTest extends TestCase {
	...
	/**
	 * @test
	 * F-020 — class-admin.php must not expose EMCP_TOOLS_DIR in localized JS data.
	 *
	 * The full filesystem path should never be sent to the browser.
	 * Only the filename ('mcp-proxy.mjs') should be localized.
	 *
	 * This test FAILS before the fix (EMCP_TOOLS_DIR is concatenated in the
	 * wp_localize_script data). After the fix it PASSES.
	 *
	 * @group security
	 * @group f-020
	 */
	public function test_localized_script_does_not_use_emcp_tools_dir_for_proxy_path(): void {
		if ( ! file_exists( $this->admin_file ) ) {
			$this->markTestSkipped( 'class-admin.php not found.' );
		}

		// The bug: EMCP_TOOLS_DIR concatenated with the proxy filename
		// in the wp_localize_script data array.
		$has_full_path_in_localize = (bool) preg_match(
			'/wp_localize_script[^;]*EMCP_TOOLS_DIR[^;]*mcp-proxy\.mjs/s',
			$this->admin_src
		);
	...
	/**
	 * @test
	 * F-020 — EMCP_TOOLS_DIR must not appear inside any wp_localize_script call.
	 *
	 * @group security
	 * @group f-020
	 */
	public function test_emcp_tools_dir_not_passed_to_localize_script(): void {
		if ( ! file_exists( $this->admin_file ) ) {
			$this->markTestSkipped( 'class-admin.php not found.' );
		}

		// Count how many wp_localize_script calls reference EMCP_TOOLS_DIR.
		$matches = [];
		preg_match_all(
			'/wp_localize_script\s*\([^;]*EMCP_TOOLS_DIR[^;]*/s',
			$this->admin_src,
			$matches
		);
		...
		$this->assertCount(
			0,
			$matches[0],
			'F-020: EMCP_TOOLS_DIR must not be used inside wp_localize_script(). ' .
	 ...
	 *
	 * This verifies that EMCP_TOOLS_DIR may still be used for server-side
	 * file operations — only its exposure to JavaScript is the bug.
	 *
	 * @group security
	 * @group f-020
	 */
	public function test_emcp_tools_dir_constant_is_defined(): void {
		// In the bootstrap, EMCP_TOOLS_DIR is defined as the plugin root.
		$this->assertTrue(
			defined( 'EMCP_TOOLS_DIR' ),
			'EMCP_TOOLS_DIR must be defined (used for server-side path construction).'
		);

		$dir = EMCP_TOOLS_DIR;
		$this->assertIsString( $dir, 'EMCP_TOOLS_DIR must be a string.' );
		$this->assertNotEmpty( $dir, 'EMCP_TOOLS_DIR must not be empty.' );
	}
