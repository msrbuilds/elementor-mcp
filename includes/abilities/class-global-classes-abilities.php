 ...
 *
 * Ported from upstream msrbuilds/elementor-mcp (#55, with the #57
 * defensive-resolve fix), adapted to this fork's class/helper naming (the
 * upstream rename to emcp-tools is not adopted).
 *
 * @package Elementor_MCP
 * @link    https://github.com/msrbuilds/elementor-mcp/issues/55
 */
 ...
 */
class Elementor_MCP_Global_Classes_Abilities {
	...
	/**
	 * Registers the Global Classes abilities.
	 */
	public function register(): void {
		...
		elementor_mcp_register_ability(
			...
			array(
				'label'               => __( 'List Global Classes', 'elementor-mcp' ),
				'description'         => __( 'Resolves Elementor Class Manager (Global Classes) entries. Maps the opaque "g-" class IDs that appear on elements back to their human-readable names and the CSS properties they define, per breakpoint/state. Use it to understand what styling a g- class applies. Pass class_ids to resolve specific IDs, or omit to list them all. Read-only.', 'elementor-mcp' ),
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'class_ids' => array(
							...
							'description' => __( 'Optional list of g- class IDs to resolve (e.g. ["g-037bb9c"]). Omit to return every global class.', 'elementor-mcp' ),
						),
					),
				),
				'output_schema'       => array(
	...
	/**
	 * Permission check for read-only Global Classes queries.
	 *
	 * Read access is gated on `edit_posts`.
	 *
	 * @return bool
	 */
		...
		foreach ( $items as $key => $item ) {
			...
			try {
				...
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					...
					error_log( '[Elementor MCP] list-global-classes: could not fully resolve class "' . $id . '": ' . $e->getMessage() );
				}
	...
	private function flatten_variants( array $variants ): array {
		...
		foreach ( $variants as $variant ) {
			...
			foreach ( $props as $prop_name => $prop_value ) {
				...
				try {
					$flat[ (string) $prop_name ] = class_exists( 'Elementor_MCP_Atomic_Props' )
						? Elementor_MCP_Atomic_Props::unwrap( $prop_value )
						: $prop_value;
				} catch ( \Throwable $e ) {
