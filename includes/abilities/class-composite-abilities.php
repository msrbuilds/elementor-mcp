	...
	private function register_build_page(): void {
		wp_register_ability(
			...
			array(
				...
				'description'         => __( 'Creates a complete Elementor page from a declarative structure in a single call. Supports nested containers and any widget types. Settings are auto-normalised (companion keys added, bare numbers converted to {size,unit} objects). LAYOUT: (1) Side-by-side columns = parent container flex_direction=row, children auto-get content_width=full + equal % widths. (2) Never set flex_wrap or _flex_size. EXACT SETTINGS REFERENCE — Containers: background_background=classic + background_color=#hex | background_image={url,id} + background_size=cover | border_border=solid + border_color=#hex + border_width={size:1,unit:px} | border_radius={top,right,bottom,left,unit} | box_shadow_box_shadow_type=yes + box_shadow_box_shadow={horizontal,vertical,blur,spread,color} | padding={top,right,bottom,left,unit} | margin={top,right,bottom,left,unit} | min_height={size:400,unit:px} | flex_justify_content=center|flex-start|flex-end|space-between|space-around | align_items=center|flex-start|flex-end | flex_direction=row|column | column_gap={size:20,unit:px} | row_gap={size:20,unit:px} | html_tag=section|div|header|footer. Widgets — heading: title="text", header_size=h1-h6, align=left|center|right, typography_typography=custom + typography_font_size={size:48,unit:px} + typography_font_weight=700, color=#hex | text-editor: editor="<p>text</p>", align, color, typography_* | button: text="Label", link={url,is_external}, align, background_color, button_text_color, border_radius={size:4,unit:px}, typography_* | image: image={id,url}, image_size=full|large|medium, align, caption_source=custom + caption="alt text". USE inject-page-css tool for pixel-perfect CSS tweaks (animations, pseudo-elements, custom fonts). Use search-images + sideload-image before building pages with real images.', 'elementor-mcp' ),
