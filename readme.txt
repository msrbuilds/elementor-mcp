...
* **ACF / ACF PRO (beyond Elementor)** — Read and write Advanced Custom Fields values on posts and options pages, discover field groups, and author field groups programmatically. Full Pro field support (repeater, flexible content, gallery, group, clone) as nested JSON. Only registers when ACF is active; the 3 write tools ship disabled-by-default; no deletes and no field renames. (v3.3.0)
...
= 3.3.0 =
ACF / ACF PRO support.
* Added: 7 ACF tools that register only when Advanced Custom Fields (free or PRO) is active — list-acf-field-groups, get-acf-field-group, list-acf-options-pages, get-acf-fields (reads, enabled by default), update-acf-fields, create-acf-field-group, update-acf-field-group (writes, disabled-by-default; opt in on the Tools tab).
...
* Added: writes always resolve field names to field keys before calling update_field, so values persist on posts that never had the field saved before.
* Added: conservative authoring guardrails — no delete tool, fields can never be removed, a field's name/key/type can never change, and code-registered (acf-json/PHP) groups are refused by the group editor.
* Added: "ACF (Custom Fields)" category on the Tools tab (WordPress sub-tab) with per-tool toggles.
* Added: a standalone public PHPUnit harness (tests/) so outside contributors can run the ACF suite without the private pro/tests submodule: vendor/bin/phpunit -c tests/phpunit.xml.
